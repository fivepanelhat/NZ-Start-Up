"""G9 — Model tier routing + optional per-company monthly token budget."""
...
# Default skill → tier map (overridden by skill frontmatter model_tier when present)
...
# Heuristic monthly budgets (tokens) — soft warning only
...
def record_usage(
    ...
    *,
    ...
    data["last_tier"] = resolve_tier(skill, model_tier)
    save_budget(company_id, data)
    return data


def skill_frontmatter_tiers() -> dict[str, str]:
    """Scan skills/*/SKILL.md for model_tier in YAML frontmatter."""
