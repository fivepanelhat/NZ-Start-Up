...
def cmd_operate(args: argparse.Namespace) -> int:
    """First-principles digital employee brief (draft/prepare only)."""
    from nz_startup import operator

    path, state = operator.write_operator_brief(args.company_id)
    if args.json:
        # drop huge snippets for JSON board view
        slim = {
            k: v
            for k, v in state.items()
            if k not in ("profile_snippet", "runway_snippet", "status")
        }
        slim["status_score"] = (state.get("status") or {}).get("score")
        slim["status_band"] = (state.get("status") or {}).get("band")
        slim["written"] = str(path)
        print(json.dumps(slim, indent=2, default=str))
    else:
        print(operator.format_operator_brief_markdown(state))
        print(f"---\nwritten: {path}")
        print(f"latest:  {path.parent / 'brief-latest.md'}")
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # First-principles digital employee
    op = sub.add_parser(
        "operate",
        aliases=["employee"],
        help="First-principles operator brief (digital employee cadence)",
    )
    op.add_argument("company_id")
